...
export default function ExplanationCard({ targetWebsite, targetWebsiteScore, status }) {
    ...
    <CardWrapper>
      ...
      {status === "success" ? (
        <ExplCardButton onClick={handleOpen}>
          ...
          <br />
          <span>{`${targetWebsite}의 총점은 ${+targetWebsiteScore[6]?.toFixed(1)}점입니다.`}</span>
          <br />
          <span>
            {config.evaluation[0]} 점수는 <b>{targetWebsiteScore[0]}점</b>, {config.evaluation[1]}{" "}
            점수는 <b>{targetWebsiteScore[1]}점</b>, {config.evaluation[2]} 점수는{" "}
            <b>{targetWebsiteScore[2]}점</b>, {config.evaluation[3]} 점수는{" "}
            <b>{targetWebsiteScore[3]}점</b>
            입니다.
          </span>
        </ExplCardButton>
      ) : (
