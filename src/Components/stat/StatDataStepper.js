    ...
    <div className='w-full flex flex-row flex-wrap'>
      ...
      >
        {Object.entries(criteriaClass)
          .filter(([subClass, val]) => subClass !== "resultScore" && subClass !== "totalScore")
...
const SubClassStepper = ({ subClass }) => {
  ...
  return (
    <div className='w-full flex flex-row'>
      <Stepper activeStep={activeStep} orientation='vertical' className='flex-1 flex-wrap '>
                ...
                <StepContent TransitionProps={{ unmountOnExit: false }}>
                  {spec !== "resultScore" && (
                    <div key={spec} className='w-full flex flex-col'>
                      <span className='text-2xl p-2 font-bold'>{val.title}</span>
  ...
  return (
    <>
      <Phrase
        ...
        subtitle={"간단히 요약된 점수입니다. 자세한 점수는 왼쪽 대시보드에서 확인할 수 있어요."}
      />
                  ...
                  <Stepper activeStep={activeStep} orientation='vertical'>
                    {classification.map((classification, idx) => (
                      <Step key={idx}>
                        ...
                        >
                          <span className='text-lg font-bold'>
                            {config.evaluation[idx]}: {classification.resultScore} 점
                          </span>
                        </StepLabel>
